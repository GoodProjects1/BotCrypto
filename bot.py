...
# ── CoinGecko — chiamate minime ───────────────────────────────────────────────
async def fetch_all_prices() -> dict:
    """Una sola chiamata per tutti i prezzi."""
    url = (
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(url, timeout=aiohttp.ClientTimeout(total=20)) as r:
                if r.status == 200:
                    return await r.json()
                logger.error(f"CoinGecko prices: HTTP {r.status}")
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(url, timeout=aiohttp.ClientTimeout(total=10)) as r:
                ...
                logger.error(f"CoinGecko single {coin_id}: HTTP {r.status}")
    except Exception as e:
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(url, timeout=aiohttp.ClientTimeout(total=15)) as r:
                ...
                logger.error(f"OHLC {coin_id}: HTTP {r.status}")
    except Exception as e:
...
async def scan_and_notify(bot: Bot):
    ...
    # 1 sola chiamata per tutti i prezzi
    prices = await fetch_all_prices()
    if not prices:
        logger.warning("Nessun dato prezzi")
        return

    # Pre-filtra i 5 più interessanti per change24h (evita troppe chiamate OHLC)
    candidates = []
    for coin_id, name, short in COINS:
        entry = prices.get(coin_id)
        if not entry: continue
        candidates.append((coin_id, name, short, entry["usd"], entry.get("usd_24h_change", 0) or 0))
...
# ── Monitor posizioni (1 chiamata sola) ───────────────────────────────────────
async def monitor_positions(bot: Bot):
    ...
    for pid, cp, reason in to_close:
        _close(data, pid, cp, reason)
    save_data(data)
...
def _close(data, pos_id, close_price, reason):
    ...
    data["closed"].insert(0, {**pos, "close_price": close_price, "close_reason": reason,
                               "pnl": round(pnl,4), "pct": round(pct,4),
...
async def cmd_compra(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if len(ctx.args) < 2:
        await update.message.reply_text("❌ Uso: `/compra LINK 100`", parse_mode="Markdown"); return
    short = ctx.args[0].upper()
    ...
    if amount > avail:
        await update.message.reply_text(f"⚠️ Disponibile: €{avail:.2f} | Richiesto: €{amount:.2f}"); return
    ...
    pos = {
        ...
        "coin_id": coin_id, "name": SHORT_TO_NAME[short], "short": short,
        "entry_price": price, "current_price": price, "amount": amount,
    ...
    await update.message.reply_text(
        f"✅ *Posizione aperta!*\n\n*{pos['name']}* ({short})\n"
        f"💵 Entry: {fp(price)}\n💶 Investito: €{amount:.2f}\n\n"
        f"🎯 Target: {fp(pos['target_price'])} (+{TARGET_PCT}%) → *+€{amount*TARGET_PCT/100:.2f}*\n"
        f"🛑 Stop: {fp(pos['stop_price'])} (-{STOP_PCT}%) → *-€{amount*STOP_PCT/100:.2f}*\n\n"
        f"_Monitoraggio ogni 5 minuti._", parse_mode="Markdown"
    )
...
async def cmd_vendi(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    pos   = next((p for p in data["positions"] if p["short"] == short), None)
    if not pos:
        await update.message.reply_text(f"❌ Nessuna posizione su {short}"); return
    ...
    pnl = pos["amount"] * pct / 100
    await update.message.reply_text(
        f"🔴 *Posizione chiusa* — {pos['name']}\n\nEntry: {fp(pos['entry_price'])} → Exit: {fp(price)}\n"
        f"{'📈'if pnl>=0 else'📉'} *{pct:+.2f}%* → *{fe(pnl)}*\n\nP&L totale: *{fe(data['pnl_total'])}*",
        parse_mode="Markdown"
    )
    ...
    if not data["positions"]:
        ...
        for p in data["positions"]:
            ...
            if price:
                ...
                lines.append(
                    f"{'🟢'if pnl>=0 else'🔴'} *{p['short']}* €{p['amount']:.0f} → {pct:+.2f}% (*{fe(pnl)}*)\n"
                    f"   {fp(p['entry_price'])} → {fp(price)}\n"
...
async def cmd_storico(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for op in data["closed"][:10]:
        ...
        lines.append(
            f"{'✅'if op['pnl']>=0 else'❌'} {r} *{op['short']}* — {op.get('closed_at','')}\n"
            f"   {op['pct']:+.2f}% → *{fe(op['pnl'])}*  |  {fp(op['entry_price'])} → {fp(op['close_price'])}"
        )
    lines.append(f"\n💰 P&L totale: *{fe(data['pnl_total'])}*")
    await update.message.reply_text("\n".join(lines), parse_mode="Markdown")
...
async def cmd_budget(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    data["budget"] = b
    save_data(data)
    await update.message.reply_text(f"✅ Budget: €{b:.2f}")
...
def main():
    ...
    for cmd, fn in [
        ("start",       cmd_start), ("aiuto",       cmd_start),
        ("prezzo",      cmd_prezzo), ("segnali",     cmd_segnali),
        ("compra",      cmd_compra), ("vendi",       cmd_vendi),
        ("portafoglio", cmd_portafoglio), ("storico", cmd_storico),
        ("budget",      cmd_budget),
    ]:
    ...
    logger.info("Bot avviato (CoinGecko v3) ✅")
